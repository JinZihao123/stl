...
//如果不考虑多线程的话，是否二级配置器也可以做成不是template FIXME
    ...
    private:
        ...
        static char *start_memory_poll;//内存池的起始点
        static char *end_memory_poll;//内存池的终点
    ...
    _default_alloc_template::obj *volatile _default_alloc_template::free_list[_NFREELISTS] =
            {nullptr, nullptr, nullptr, nullptr,
             nullptr, nullptr, nullptr, nullptr,
             nullptr, nullptr, nullptr, nullptr,
             nullptr, nullptr, nullptr, nullptr};
    char *_default_alloc_template::start_memory_poll = nullptr;//内存池的起始点
    char *_default_alloc_template::end_memory_poll = nullptr;//内存池的终点
    ...
    void *_default_alloc_template::refill(size_t n) {
        ...
        *my_free_list = reinterpret_cast<obj *>(chunk + n);//+n就是下一个块的地址头部了
        obj *next = reinterpret_cast<obj *>(chunk + n);
        obj *cur;
        for (int i = 1;; i++) {
            cur = next;
    ...
    char *_default_alloc_template::chunk_alloc(size_t size, int &nobjs) {
        ...
        if (left_bytes >= need_bytes) {
            ...
            if (left_bytes > 0) {
                ...
                reinterpret_cast<obj *> (start_memory_poll)->fress_list_link = *my_free_list;
                *my_free_list = reinterpret_cast<obj *>(start_memory_poll);
            }
            ...
            if (start_memory_poll == nullptr) {
                ...
                for (int i = size; i <= _MAX_BYTES; i += _ALIGN) {
                    ...
                    if (p != nullptr) {
                        *my_free_list = p->fress_list_link;
                        start_memory_poll = reinterpret_cast<char *>(p);
                ...
                start_memory_poll = reinterpret_cast<char *>(__malloc_alloc_template::allocate(bytes_to_get));
            }
